...
{
    public class UpdatePostContentsTests
    {
        ...
        public UpdatePostContentsTests()
        {
            ...
            _mockDbCommand.Setup(c => c.GetParameterValue("@RowsAffected"))
                          .Returns(1);
        ...
        [Fact]
        public async Task UpdatePostContentsAsync_ValidPost_ReturnsSuccess()
        {
            // Arrange
            var post = Post.Create(Guid.NewGuid(), Guid.NewGuid(), "Valid Title", "Valid Content", "https://example.com").Data;
            post.UpdatePostContents("Updated Title", "Updated Content");
            var cancellationToken = new CancellationToken();

            // Act
            var result = await _postRepository.UpdatePostContentsAsync(post, cancellationToken);
        ...
        [Fact]
        public async Task UpdatePostContentsAsync_NoRowsAffected_ReturnsFailure()
        {
            // Arrange
            var post = Post.Create(Guid.NewGuid(), Guid.NewGuid(), "Valid Title", "Valid Content", "https://example.com").Data;
            post.UpdatePostContents("Updated Title", "Updated Content");
            ...
            _mockDbCommand.Setup(c => c.GetParameterValue("@RowsAffected")).Returns(0);
            ...
            Assert.Contains("Update Failed", result.Errors.First().Message);
            _mockLogger.Verify(l => l.LogWarning(It.IsAny<string>(), It.IsAny<object[]>()), Times.Once);
        }

        [Fact]
        public async Task UpdatePostContentsAsync_CancelledOperation_ReturnsCancellationHandled()
        {
            // Arrange
            var post = Post.Create(Guid.NewGuid(), Guid.NewGuid(), "Valid Title", "Valid Content", "https://example.com").Data;
            post.UpdatePostContents("Updated Title", "Updated Content");
        ...
        [Fact]
        public async Task UpdatePostContentsAsync_ExceptionOccurs_ReturnsFailure()
        {
            // Arrange
            var post = Post.Create(Guid.NewGuid(), Guid.NewGuid(), "Valid Title", "Valid Content", "https://example.com").Data;
            post.UpdatePostContents("Updated Title", "Updated Content");
